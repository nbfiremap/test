...
const legendURLForLayer = (fullyQualifiedLayer)=>{
 ...
 topbar.append(baseCtl);
      ...
      function collectFireRows() {
        ...
        for (const rec of fireStore.values()) {
          ...
          const base = {
            ...
            Extinguished_at: outMs != null ? new Date(outMs).toISOString() : null,
      ...
      async function exportSummaryPDF() {
        ...
        doc.text('New Brunswick Fire Summary', marginX, y); y += 18;
        doc.setFont('helvetica','normal'); doc.setFontSize(10);
        doc.text(`Generated: ${new Date(stats.generatedAt).toLocaleString()}`, marginX, y); y += 18;

        // Stats table
        const statEntries = [
          ['Total Fires', String(stats.totalFires)],
          ['Active Fires', String(stats.activeFires)],
          ['Extinguished Fires', String(stats.extinguishedFires)],
          ['Detected Today', String(stats.detectedToday)],
          ['Detected Yesterday', String(stats.detectedYesterday)],
          ['Out Today', String(stats.outToday)],
          ['Out Yesterday', String(stats.outYesterday)],
          ['Area Burned (ha)', toNum(stats.areaHaTotal,1)]
        ];
        doc.autoTable({
          startY: y,
          head: [['Metric', 'Value']],
        ...
        // Status breakdown
        const statusBody = Object.entries(stats.byStatus)
        ...
        // Full fires list (key columns; keeps PDF reasonable). Excel has the complete property dump.
        ...
        doc.autoTable({
          startY: y,
          head: [['Fire', 'Status', 'ha', 'Detected', 'Out', '% Cont.', 'X', 'Y']],
          body: fireBody,
          styles: { fontSize: 8, cellPadding: 3 },
          headStyles: { fontStyle:'bold' },
          columnStyles: { 2: { halign: 'right' }, 6: { halign: 'right' }, 7: { halign: 'right' } },
          didDrawPage: (data) => {
            // Simple footer
